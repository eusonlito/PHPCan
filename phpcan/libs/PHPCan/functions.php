...
function getDatabaseObject ($connection = null)
{
    ...
    if ($settings['type'] === 'mysql') {
        return new \PHPCan\Data\Databases\Mysql($connection);
    }
...
function getImageObject ()
{
    return new \PHPCan\Files\Images\Image;

    global $Config;

    if ($Config->images['library'] === 'imagick') {
        return new \PHPCan\Files\Images\Imagick;
    } else {
        return new \PHPCan\Files\Images\Gd;
    }
}
...
{
    return new \PHPCan\I18n\Datetime($time, $timezone);
}
    ...
    foreach ((array) $folders as $folder) {
        ...
        if (is_dir($folder)) {
            ...
            if (!is_object($Gettext)) {
                $Gettext = new \PHPCan\I18n\Gettext;
            }
