...
class Table
{
     ...
     * return none
     */
    public function __construct (\PHPCan\Data\Db $Db, $table, $table_config, $relation_config)
    {
        global $Debug;
    ...
    private function setFormatsConfig ()
    {
        ...
        $this->formats['id'] = new \PHPCan\Data\Formats\Id($this->table, 'id', $this->Db->languages);
        ...
        foreach ($this->settings['table'] as $field => $settings) {
            ...
            $class_name = '\\PHPCan\\Data\\Formats\\'.ucfirst($settings['format']);
    ...
    private function setRelationsConfig ()
    {
        ...
        foreach ((array) $this->settings['relations'] as $name => $settings) {
            $class_name = '\\PHPCan\\Data\\Relations\\Relation_'.str_replace(' ', '_', $settings['mode']);
