...
$VERSION='$Id: content-main.php 527 2011-11-06 02:02:28Z lito $';
...
h1.apc { background:#CC3300; margin:0; padding:0.5em 1em 0.5em 1em; }
...
hr.apc {
    background:white;
    border-bottom:solid #662200 1px;
    border-style:none;
    border-top:solid #662200 10px;
    height:12px;
    margin:0;
    margin-top:1px;
    padding:0;
...
div.info table td.td-0 { border-right:solid #662200 1px; white-space:nowrap; }
div.info table td.td-n { border-right:solid #662200 1px; }
...
if ($AUTHENTICATED) {
    echo <<<EOB
        <li><a href="$MY_SELF&CC=1" onClick="javascript:return confirm('Are you sure?');">Clear Cache</a></li>
EOB;
}
echo <<<EOB
    </ol>
...
EOB;
  ...
  echo
    ...
    foreach ($cache[$scope_list[$MYREQUEST['SCOPE']]] as $i => $entry) {
        ...
        if (!strstr($entry[$fieldname], BASE_PATH)) {
            continue;
        }
        ...
        if (!$AUTHENTICATED) {
            ...
            $entry[$fieldname] = str_replace(BASE_PATH, '' , $entry[$fieldname]);
...
EOB;
    ...
    foreach ($cache[$scope_list[$MYREQUEST['SCOPE']]] as $entry) {
        if (!strstr($entry['filename'], BASE_PATH)) {
            continue;
        }
        $n = str_replace(BASE_PATH, '', dirname($entry['filename']).'/');
        if ($MYREQUEST['AGGR'] > 0) {
